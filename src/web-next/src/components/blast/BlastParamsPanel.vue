    ...
    <div class="form-group">
      ...
      <div class="select-box-neo" @click.stop="emit('toggleDropdown', 'db', $event)">
         ...
         <div v-if="openDropdown === 'db'" class="dropdown-list">
            ...
            <div v-for="o in DB_OPTIONS.nucleotide" :key="o.value" class="opt" @click="emit('selectOption', 'database', o.value)">{{ o.label }}</div>
            <div class="group">蛋白</div>
            <div v-for="o in DB_OPTIONS.protein" :key="o.value" class="opt" @click="emit('selectOption', 'database', o.value)">{{ o.label }}</div>
         </div>
      </div>
    </div>
    <div class="form-row">
    ...
    <div class="form-group">
       ...
       <div class="select-box-neo" @click.stop="emit('toggleDropdown', 'matrix', $event)">
          ...
          <div v-if="openDropdown === 'matrix'" class="dropdown-list">
            <div v-for="o in MATRIX_OPTIONS" :key="o.value" class="opt" @click="emit('selectOption', 'matrix', o.value)">{{ o.label }}</div>
          </div>
       </div>
    </div>
    <div class="form-row">
...
.section-title.sub { margin-top: 24px; font-size: 0.8rem; opacity: 0.7; }

.form-group { margin-bottom: 16px; }
.form-group label { display: block; font-size: 0.75rem; font-weight: 600; color: #64748b; margin-bottom: 6px; }
.form-row { display: grid; grid-template-columns: 1fr 1fr; gap: 12px; }

.neo-input { width: 100%; background: #f8fafc; border: 1px solid #e2e8f0; border-radius: 8px; padding: 8px 12px; font-size: 0.85rem; color: #334155; outline: none; transition: all 0.2s; }
.neo-input:focus { border-color: #2563eb; background: white; box-shadow: 0 0 0 3px rgba(37,99,235,0.1); }

.select-box-neo { width: 100%; background: #f8fafc; border: 1px solid #e2e8f0; border-radius: 8px; padding: 8px 12px; font-size: 0.85rem; color: #334155; cursor: pointer; position: relative; display: flex; align-items: center; justify-content: space-between; }
.select-box-neo:hover { border-color: #cbd5e1; }
.select-box-neo .arrow { font-size: 0.6rem; opacity: 0.5; }

.dropdown-list { position: absolute; top: calc(100% + 4px); left: 0; right: 0; background: white; border: 1px solid #e2e8f0; border-radius: 10px; box-shadow: 0 10px 25px rgba(0,0,0,0.1); z-index: 50; max-height: 250px; overflow-y: auto; overflow-x: hidden; padding: 6px; }
.dropdown-list .opt { padding: 8px 12px; font-size: 0.82rem; border-radius: 6px; color: #475569; }
.dropdown-list .opt:hover { background: #f1f5f9; color: #2563eb; }
.dropdown-list .group { padding: 8px 12px; font-size: 0.7rem; font-weight: 800; color: #94a3b8; text-transform: uppercase; letter-spacing: 0.05em; background: #f8fafc; margin: 4px -6px; pointer-events: none; }

.checkbox-group { display: flex; align-items: center; gap: 8px; margin-top: 8px; }
.checkbox-group input { width: 16px; height: 16px; accent-color: #2563eb; cursor: pointer; }
