...
function commitRename(task: any) {
  if (editingTaskId.value === task.taskId) {
    if (editName.value.trim() && editName.value !== task.fileName) {
      task.fileName = editName.value.trim()
      try {
        getBridge().rename_task(task.taskId, task.fileName)
      } catch { }
    }
    ...
    <div class="left-col">
      ...
      <div class="panel input-panel">
        ...
        <div class="panel-body">
          ...
          <!-- Text 模式 -->
          <div v-else>
            <textarea v-model="blast.queryText" class="query-textarea"
          ...
          <div class="form-group">
            ...
            <select v-model="blast.params.database" class="form-input">
              <optgroup label="核酸数据库">
                <option v-for="opt in DB_OPTIONS.nucleotide" :key="opt.value" :value="opt.value">{{ opt.label }}</option>
              </optgroup>
      ...
      <div class="panel history-panel" :class="{ collapsed: !blast.historyVisible }">
        ...
        <div v-if="blast.historyVisible" class="panel-body scroll-v">
          <div v-for="task in blast.tasks" :key="task.taskId" class="task-item"
               :class="{ active: blast.activeTaskId === task.taskId }"
            ...
            <div class="task-name" @dblclick="startRename(task, $event)" title="双击重命名">
              ...
              <input v-else v-model="editName" class="rename-input"
                     @blur="commitRename(task)" @keyup.enter="commitRename(task)" @click.stop/>
            </div>
            <div class="task-meta">
            ...
            <div class="task-actions">
              <button v-if="task.status === 'running'" class="btn-icon sm" @click.stop="pauseTask(task.taskId)" title="暂停">⏸</button>
              <button v-if="task.status === 'running'" class="btn-icon sm" @click.stop="stopTask(task.taskId)" title="停止">⏹</button>
...
.blast-view {
  ...
  background: #f1f5f9;
  position: relative;
  overflow: hidden; /* Ensure split styling */
}
/* ... */
.history-tag {
  position: absolute;
...
.history-tag:hover { background: #f8fafc; transform: translateY(-50%) translateX(-2px); }
...
.mode-tab.active { background: var(--accent-blue); color: white; }
...
.drop-zone:hover { border-color: var(--accent-blue); background: rgba(59, 130, 246, 0.03); }
...
.file-name { color: var(--text-primary); font-family: monospace; font-size: 0.78rem; }
...
.form-group label { display: block; font-size: 0.78rem; font-weight: 600; margin-bottom: 4px; color: var(--text-primary); }
.form-input {
  width: 100%;
  padding: 7px 10px;
  border: 1px solid var(--border-color);
...
/* 历史面板 */
.history-panel { width: 240px; min-width: 240px; }
.history-panel.collapsed { width: 0; min-width: 0; overflow: hidden; }
...
.task-item.active { background: rgba(59, 130, 246, 0.05); border-left: 3px solid var(--accent-blue); }
.task-name { font-size: 0.82rem; font-weight: 500; margin-bottom: 4px; }
.task-meta { display: flex; align-items: center; gap: 6px; }
.task-status { font-size: 0.72rem; padding: 2px 6px; border-radius: 4px; }
.task-status.queued { background: #f1f5f9; color: #64748b; }
.task-status.running { background: #dbeafe; color: #2563eb; }
.task-status.done { background: #d1fae5; color: #059669; }
.task-status.error { background: #fee2e2; color: #dc2626; }
.task-status.paused { background: #fef3c7; color: #d97706; }
...
.accession-link { color: var(--accent-blue); cursor: pointer; text-decoration: none; }
.accession-link:hover { text-decoration: underline; }

/* 鉴定详情列 3 行布局 */
.query-col { max-width: 160px; overflow: hidden; text-overflow: ellipsis; white-space: nowrap; font-family: monospace; font-size: 0.78rem; }
.hit-detail-col { min-width: 180px; }
.hit-species { font-weight: 700; font-size: 0.88rem; color: var(--text-primary); line-height: 1.4; }
.hit-strain { font-size: 0.74rem; color: var(--text-muted); margin-top: 2px; }
.hit-source { font-size: 0.72rem; color: var(--accent-blue); margin-top: 2px; }

/* 生物学背景列 */
.bio-col { min-width: 100px; }
.bio-tag { font-size: 0.74rem; color: var(--text-secondary); }
.bio-host { font-size: 0.72rem; color: var(--text-muted); margin-top: 2px; }
.bio-len { font-size: 0.72rem; color: var(--text-muted); margin-top: 2px; }

.btn-icon { background: none; color: var(--text-muted); padding: 4px; border-radius: 4px; font-size: 0.85rem; }
.btn-icon:hover { background: #f1f5f9; }
.btn-icon.sm { font-size: 0.75rem; }

.empty-state { text-align: center; padding: 100px 20px; color: var(--text-muted); }
.empty-icon { font-size: 3rem; opacity: 0.3; margin-bottom: 12px; }
.empty-hint { text-align: center; padding: 20px; color: var(--text-muted); font-size: 0.82rem; }
...
.history-tag:hover { background: #f1f5f9; }
</style>
