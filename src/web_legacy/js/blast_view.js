...
class BlastViewController {
    constructor() {
        ...
        this.dom = {
            ...
            dictCategory: document.getElementById('edit-category-select'),
    ...
    async init() {
        ...
        // Use global bridge from app
        this.localBridge = window.py_bridge;

        if (this.localBridge) {
            // Connect to shared signals
            this.localBridge.blast_event.connect((type, data) => {
                this.onBlastEvent({ type, data });
            });
    ...
    /**
     * Handle incoming blast events (called from app.js or local signal)
     */
    onBlastEvent(event) {
        const { type, data } = event;
        console.log(`[BlastView] Bridge Event: ${type}`, data);
    ...
    appendResultRow(res) {
        ...
        row.dataset.csvFile = res.csv_file;
        row.dataset.xmlFile = res.xml_file;
    ...
    _fillRowContent(row, res) {
        ...
        const speciesDisplayName = bestHit.species || '未知物物种';
        row.innerHTML = `
            <td style="font-weight: 500; font-size: 0.8rem; color: var(--text-muted);">${res.sequence_id}</td>
            <td>
                <div class="species-name" style="font-weight: 700; color: var(--text-main); font-size: 0.85rem; white-space: nowrap; overflow: hidden; text-overflow: ellipsis; max-width: 180px;" title="${speciesDisplayName}" data-english="${bestHit.species}">${speciesDisplayName}</div>
            ...
            <td>
                <div style="display: flex; gap: 8px; align-items: center;">
                    <button class="btn-icon translate-row-btn" title="AI 翻译物种名" onclick="window.blastView.translateRowByBtn(this)" style="opacity: 0.6;">
                    ...
                    <button class="btn-icon" title="查看比对详情" onclick="window.blastView.viewDetailByBtn(this, '${res.sequence_id}')">
    ...
    translateRowByBtn(btn) {
        const row = btn.closest('tr');
        const species = row.querySelector('.species-name').dataset.english;
        this.translateRow(btn, species);
    }

    viewDetailByBtn(btn, seqId) {
        const row = btn.closest('tr');
        this.viewDetail(row.dataset.csvFile, row.dataset.xmlFile, seqId);
    }
    ...
    closeDetail() {
        if (this.dom.detailOverlay) this.dom.detailOverlay.classList.remove('active');
        if (this.dom.dictOverlay) this.dom.dictOverlay.classList.remove('active');
        if (this.dom.detailBody) this.dom.detailBody.innerHTML = '';
        this.currentDetailXml = null;
    }
    ...
    renderDetailHits(hits) {
        ...
        hits.forEach((hit, idx) => {
            ...
            // Rich biological metadata assembly
            const bioInfo = [];
            if (hit.genus) bioInfo.push(`属名: <span style="color:var(--text-main); font-weight:600;">${hit.genus}</span>`);
            if (hit.strain) bioInfo.push(`菌株: <span style="color:var(--text-main); font-weight:600;">${hit.strain}</span>`);
            if (hit.gene_type) bioInfo.push(`基因: <span style="color:var(--primary); font-weight:600;">${hit.gene_type}</span>`);
            if (hit.host) bioInfo.push(`宿主: <span style="color:var(--text-main);">${hit.host}</span>`);
            ...
            card.innerHTML = `
                <div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 12px;">
                    <div style="font-weight: 700; color: var(--text-main); font-size: 1rem; display: flex; align-items: center; gap: 8px;">
                        ...
                        <span class="species-name" data-english="${hit.species}">${hit.species || '未知物种'}</span>
                        <div class="translated-name" style="font-size: 0.8rem; color: var(--primary); font-weight: 600; display: none; margin-left: 4px;"></div>
                        <button class="btn-icon" title="AI 翻译" onclick="window.blastView.translateDetailHit(this)" style="opacity: 0.6; padding: 2px;">
                            <svg class="icon-svg" viewBox="0 0 24 24" style="width: 14px; height: 14px;"><path d="M5 8l6 6"></path><path d="M4 14l6-6 2-3"></path><path d="M2 5h12"></path><path d="M22 22l-5-10-5 10"></path><path d="M14 18h6"></path></svg>
                        </button>
                    </div>
                    <span class="badge" style="background: ${sim > 98 ? '#ecfdf5' : '#f8fafc'}; color: ${sim > 98 ? '#065f46' : '#64748b'}; border: 1px solid ${sim > 98 ? '#ccfbf1' : '#e2e8f0'}; font-weight:700;">
                        相似度: ${hit.similarity}
                    </span>
                </div>
                
                <div style="font-size: 0.78rem; color: #64748b; background: #f1f5f9; padding: 10px; border-radius: 6px; margin-bottom: 15px; border-left: 3px solid var(--primary);">
                    ${hit.title}
                </div>

                <div style="display: flex; flex-wrap: wrap; gap: 15px 30px; margin-bottom: 18px; padding: 0 5px;">
                    ${bioInfo.map(info => `<div style="font-size: 0.85rem; color: var(--text-muted);">${info}</div>`).join('')}
                </div>

                <div class="stat-grid" style="background: #fff; border: 1px solid #f1f5f9; border-radius: 10px; padding: 12px; display: grid; grid-template-columns: repeat(4, 1fr); gap: 10px;">
                    <div class="stat-item"><div class="stat-label" style="font-size:0.7rem; color:var(--text-muted); margin-bottom:4px;">E-Value</div><div class="stat-value" style="font-family:'Fira Code', monospace; font-size:0.85rem; color:var(--text-main); font-weight:600;">${hit.evalue}</div></div>
                    <div class="stat-item"><div class="stat-label" style="font-size:0.7rem; color:var(--text-muted); margin-bottom:4px;">Accession</div><div class="stat-value" style="color:var(--primary); font-family:'Fira Code', monospace; font-size:0.85rem; font-weight:600;">${hit.acc}</div></div>
                    <div class="stat-item"><div class="stat-label" style="font-size:0.7rem; color:var(--text-muted); margin-bottom:4px;">Identities</div><div class="stat-value" style="font-size:0.85rem; color:var(--text-main);">${hit.ident_count}/${hit.align_len}</div></div>
                    <div class="stat-item"><div class="stat-label" style="font-size:0.7rem; color:var(--text-muted); margin-bottom:4px;">Gaps</div><div class="stat-value" style="font-size:0.85rem; color:var(--text-main);">${hit.gaps}</div></div>
                </div>

                <div style="margin-top: 15px; font-size: 0.72rem; color: var(--text-muted); display:flex; justify-content: space-between; background: #fafafa; padding: 8px 12px; border-radius: 6px; border: 1px dashed #e2e8f0;">
                    <span>查询区间: <b style="color:var(--text-main);">${hit.query_range}</b></span>
                    <span>比对区间: <b style="color:var(--text-main);">${hit.hit_range}</b></span>
                    <span>序列长度: <b style="color:var(--text-main);">${hit.len}</b></span>
                </div>`;
    ...
    translateRow(btn, text) {
        if (!this.localBridge) return;
        const container = btn.closest('tr, .hit-card');
        const targetEl = container?.querySelector('.translated-name');
        if (!targetEl) return;
        ...
        this.localBridge.translate_text(text, "species", (translated) => {
            // Defensive: ensure button still exists
            if (!btn || !btn.parentElement) return;

            // If button is inside detailOverlay, ensure it's still active
            const isInsideDetail = this.dom.detailOverlay.contains(btn);
            if (isInsideDetail && !this.dom.detailOverlay.classList.contains('active')) return;
            if (translated && translated !== text) {
                let isAI = translated.startsWith('[AI]');
                let cleanText = isAI ? translated.substring(4) : (translated.startsWith('[本地]') ? translated.substring(4) : translated);

                if (isAI) {
                    targetEl.innerHTML = `
                        <span class="text-content">${cleanText}</span>
                        <div class="confirm-actions" style="display: inline-flex; gap: 6px; margin-left: 8px; vertical-align: middle;">
                            <button class="btn-tiny" onclick="window.blastView.confirmTranslation(this, '${text.replace(/'/g, "\\'")}', '${cleanText.replace(/'/g, "\\'")}')" title="✅ 确认并存入词库" style="border:none; background:none; cursor:pointer; font-size:12px; padding:2px;">✅</button>
                            <button class="btn-tiny" onclick="window.blastView.editTranslation('${text.replace(/'/g, "\\'")}', '${cleanText.replace(/'/g, "\\'")}', 'species')" title="✏️ 手动修正" style="border:none; background:none; cursor:pointer; font-size:12px; padding:2px;">✏️</button>
                        </div>
                    `;
                } else {
                    targetEl.innerText = cleanText;
                }
            ...
            setTimeout(() => {
                ...
                btn.style.opacity = isTranslated ? "1" : "0.6";
                if (isTranslated) btn.style.color = "var(--primary)";
            }, 1000);
        });
    }

    confirmTranslation(btn, english, chinese) {
        if (!this.localBridge?.update_dictionary_entry || !btn) return;
        const originalHtml = btn.innerHTML;
        btn.innerHTML = "⏳";
        btn.disabled = true;
        this.localBridge.update_dictionary_entry(english, chinese, 'species', (success) => {
            // Defensive: Detail view might have been closed/body cleared
            if (!btn || !btn.parentElement) {
                console.log("Translation confirm returned but button is detached.");
                return;
            }
            if (success) {
                const parent = btn.parentElement;
                parent.innerHTML = '<span style="color:#10b981; font-size:0.75rem; font-weight:bold;">已存库</span>';
            } else {
                btn.innerHTML = originalHtml;
                btn.disabled = false;
                console.error("Translation confirm failed for:", english);
            }
        });
    }

    translateDetailHit(btn) {
        const container = btn.closest('.hit-card');
        const speciesNameEl = container.querySelector('.species-name');
        const englishText = speciesNameEl.dataset.english;
        this.translateRow(btn, englishText);
    }
    ...
    renderDictResults(results) {
        ...
        results.forEach(item => {
            ...
            div.innerHTML = `
                ...
                <button class="btn btn-ghost" onclick="window.blastView.editTranslation('${item.english.replace(/'/g, "\\'")}', '${item.chinese.replace(/'/g, "\\'")}', '${item.category}')" style="padding: 4px 10px;">修改</button>`;
            this.dom.dictResults.appendChild(div);
        });
    }

    editTranslation(english, chinese, category = 'other') {
        if (this.dom.dictSource) this.dom.dictSource.value = english;
        if (this.dom.dictTarget) this.dom.dictTarget.value = chinese;
        if (this.dom.dictCategory) this.dom.dictCategory.value = category || 'other';
        this.dom.dictOverlay.classList.add('active');
    }
    ...
    saveTranslationEdit() {
        ...
        const category = this.dom.dictCategory ? this.dom.dictCategory.value : 'other';
        if (!chinese) {
            this.showNotification(this.t('msg_enter_translation'), 'warning');
            return;
        }
        if (this.localBridge?.update_dictionary_entry) {
            this.localBridge.update_dictionary_entry(english, chinese, category, (success) => {
                if (success) {
                    this.closeEditTrans();
                    this.searchDict();
                    this.showNotification(this.t('msg_save_success'), 'success');
                    document.querySelectorAll('.species-name').forEach(el => {
                        if (el.dataset.english === english) {
                            const container = el.closest('tr, .hit-card');
                            const targetEl = container?.querySelector('.translated-name');
                            if (targetEl) {
                                targetEl.innerText = chinese;
                                targetEl.style.display = 'block';
                            }
                        }
                    });
    ...
    runIntelligentRepair() {
        UIHelper.runIntelligentRepair(this.localBridge, this, 'btn-repair-dict');
    }
