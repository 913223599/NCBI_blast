...
使用 BaseAlignmentEngine 的共享路径工具，消除重复代码。
"""
...
from src.analysis.engines.engine_base import BaseAlignmentEngine


class SequenceOrientator:
    """
    序列极性校正器
    职责：通过 minimap2 快速启发式比对，判断 Query 的链方向并在必要时翻转。
    """
    ...
    async def detect_and_fix(
        self, ref_path: Path, query_path: Path, out_dir: Path
    ) -> Tuple[Path, bool]:
        """
        检测方向并生成校正后的序列文件
        ...
        w_ref = BaseAlignmentEngine.to_wsl_path(ref_path)
        w_query = BaseAlignmentEngine.to_wsl_path(query_path)

        cmd = [
            "wsl", "-d", self.wsl_distro, "-u", "root", "bash", "-c",
        ...
        try:
            ...
            w_flipped = BaseAlignmentEngine.to_wsl_path(flipped_path)
